...
#BatchNorm3d = functools.partial(InPlaceABNSync, activation='none')
        ...
        for i in range(num_branches if self.multi_scale_output else 1):
            ...
            for j in range(num_branches):
                if j > i:
                    ...
                    for k in range(i-j):
                        if k == i - j - 1:
                            num_outchannels_conv3x3x3 = num_inchannels[i]
                            conv3x3x3s.append(nn.Sequential(
                                nn.Conv3d(num_inchannels[j],
                                          num_outchannels_conv3x3x3,
                                          3, 2, 1, bias=False),
                                BatchNorm3d(num_outchannels_conv3x3x3,
                                            momentum=BN_MOMENTUM)))
                        else:
                            num_outchannels_conv3x3x3 = num_inchannels[j]
                            conv3x3x3s.append(nn.Sequential(
                                nn.Conv3d(num_inchannels[j],
                                          num_outchannels_conv3x3x3,
                                          3, 2, 1, bias=False),
        ...
        for i in range(len(self.fuse_layers)):
            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    slices_output = x[i].shape[-3]
                    y = y + F.interpolate(
                        self.fuse_layers[i][j](x[j]),
                        size=[slices_output, height_output, width_output],
                        mode='trilinear',
                        align_corners=True)
                else:
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        stem_num_channels = extra.STEM_INPLANES #64
        super(HighResolutionNet, self).__init__()

        # stem net
        self.conv1 = nn.Conv3d(config.MODEL.IN_CHANNELS, stem_num_channels, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = BatchNorm3d(stem_num_channels, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv3d(stem_num_channels, stem_num_channels, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn2 = BatchNorm3d(stem_num_channels, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=False)

        self.layer1 = self._make_layer(Bottleneck, stem_num_channels, stem_num_channels, 2)
        ...
        self.transition1 = self._make_transition_layer([stem_num_channels*4], num_channels)#256
            ...
            if i < num_branches_pre:
                ...
                for j in range(i+1-num_branches_pre):
                    ...
                    conv3x3x3s.append(nn.Sequential(
                        ...
                        nn.ReLU(inplace=False)))
                transition_layers.append(nn.Sequential(*conv3x3x3s))
    ...
    def init_weights(self, pretrained='',):
        ...
        for m in self.modules():
            ...
            elif isinstance(m, nn.BatchNorm3d):#InPlaceABNSync):
